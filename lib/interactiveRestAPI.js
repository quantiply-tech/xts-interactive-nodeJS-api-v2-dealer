...
module.exports = class XTSInteractive {
  ...
  async placeOrder(reqObject) {
    if (this.clientID) {
      reqObject = { clientID: this.clientID, ...reqObject };
    }
    try {
      await this.checkLoggedIn();
      let response = await request.processRequest(
  ...
  async modifyOrder(reqObject) {
    ...
    try {
      ...
      let response = await request.processRequest(
        ...
        this.url + settings.restApi.orders,
        { authorization: this.token },
        { clientID: this.clientID, ...reqObject }
      );
      return response;
    } catch (error) {
      let customError = new CustomError(
   ...
   */
  async getOrderBook() {
    let requestUrl = this.url + settings.restApi.orderBook;
    if (this.clientID) {
      requestUrl += '?clientID=' + this.clientID;
    }
    try {
  ...
  async getTradeBook() {
    try {
      let requestUrl = this.url + settings.restApi.tradeBook;
      if (this.clientID) {
        requestUrl += '?clientID=' + this.clientID;
      }
      await this.checkLoggedIn();
